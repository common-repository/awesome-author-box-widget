<?php
/**
 * Plugin Name: Awesome Author Box Widget
 * Plugin URI:  http://www.modinatheme.com/plugins/awesome-author-box-widget/
 * Description: Awesome Author Box widget
 * Version:     1.0
 * Author:      ModinaTheme
 * Author URI:  https://profiles.wordpress.org/modinatheme/
 * License:     GPL2
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: modinatheme
 */


// Register Widgets
add_action('widgets_init', function() {
    register_widget('Modina_author_box');
});



class Modina_author_box extends WP_Widget {

	public function __construct() {

		$widget_opt = array(
			'class'         => 'modina_author_box',
			'description'         => __('This is awesome author info widgets.'),
			'customize_selective_refresh' => true,
		);

		parent::__construct('modina_author', __('Awesome Author Box'), $widget_opt );
	}
    ...
    ?>
        ...
        <div class="sidebar_about_content">
            <h3 class="text-center sidebar_ab_title"><?php echo $instance['author_name']; ?></h3>
...
add_action('admin_enqueue_scripts', 'modina_admin_custom_script');
add_action('wp_enqueue_scripts', 'modina_admin_custom_script');


function modina_admin_custom_script() {

	wp_enqueue_media();

	wp_enqueue_style( 'font-awesome', plugins_url( 'assets/css/font-awesome.min.css', __FILE__ ) );
	wp_enqueue_style( 'animated', plugins_url( 'assets/css/animated.css', __FILE__ ) );
	wp_enqueue_style( 'modina_author_style', plugins_url( 'assets/css/style.css', __FILE__ ) );

	wp_register_script('admin-custom-js', plugins_url( 'assets/js/admin-custom.js', __FILE__ ), 'jquery');
